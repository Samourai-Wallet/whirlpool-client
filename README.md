...
```
java -jar target/whirlpool-client-0.0.1-SNAPSHOT-run.jar --network={main,test} --utxo= --utxo-key= --seed-passphrase= --seed-words= [--liquidity] [--rounds=1] [--server=host:port] [--debug]
```
...
- rounds: (1 to N) number of mixes to complete. Client will keep running (as a liquidity) until completing this number of mixes.
